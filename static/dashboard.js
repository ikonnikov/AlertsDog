function AlertChart() {
	// define chart layout
	var margin = {
		top: 80,  // top margin includes title and legend
	...
	var dataHeight = 14;  // height of horizontal data bars
	var lineSpacing = 10;  // spacing between horizontal data bars
	var paddingTopHeading = -60;  // vertical space for heading
	...
	var drawTitle = 1;  // title of chart is drawn or not (default: yes)
	var titleText = '';
	var emphasizeYearTicks = 1;  // year ticks to be emphasized or not (default: yes)
	var maxDisplayDatasets = 0;  // define chart pagination, max. no. of datasets that is displayed, 0: all (default: all)
			...
			if (definedBlocks === null) {
				...
				for (var i = 0; i < dataset.length; i++) {
					if (dataset[i].data[0].length === 3) {
						...
						if (definedBlocks) {
							throw new Error('Detected different data formats in input data. Format can either be continuous data format or time gap data format but not both.');
						}
					}
				}
			}
				...
				d.data.forEach(function (d1) {
					if (!(d1[0] instanceof Date)) {
						if (parseDateRegEx.test(d1[0])) {
							d1[0] = parseDate.parse(d1[0]);  // d1[0] is date without time data
						} else if (parseDateTimeRegEx.test(d1[0])) {
							d1[0] = parseDateTime.parse(d1[0]);  // d1[0] is date with time data
						...
						if (!definedBlocks) {
							...
							if (parseDateRegEx.test(d1[2])) {
								d1[2] = parseDate.parse(d1[2]);  // d1[2] is date without time data
							} else if (parseDateTimeRegEx.test(d1[2])) {
								d1[2] = parseDateTime.parse(d1[2]);  // d1[2] is date with time data
							} else {
								throw new Error('Date/time format not recognized. Pick between \'YYYY-MM-DD\' or \'YYYY-MM-DD HH:MM:SS\'.');
							}
						}
			...
			g.selectAll('rect')
				...
				.attr('class', function (d) {
					if (d[1] === 1) {
						...
						return 'rect_has_belated';
					} else if (d[1] === 3) {
						return 'rect_has_warning';
					} else if (d[1] === 4) {
						return 'rect_has_fail';
					} else if (d[1] === 5) {
						return 'rect_has_execution';
					} else {
						return 'rect_has_outtimed';
					}
				})
				.on('mouseover', function (d, i) {
					...
					div.html(function () {
						...
						if (d[1] === 4) {
							output = '<i class="fa fa-fw fa-times-circle-o tooltip_has_fail"></i>';  // cross icon
						} else {
							output = '<i class="fa fa-fw fa-check-circle-o tooltip_has_ok"></i>';  // checkmark icon
						}
				...
				if (emphasizeYearTicks && !(isYearTick.every(function (d) { return d === true; })) && isMonthTick.every(function (d) { return d === true; })) {
				...
				if (noOfDatasets) {
					if (isDateOnlyFormat) {
						subtitleText = 'from ' + moment(parseDate(startDate)).format('MMMM Y') + ' to ' + moment(parseDate(endDate)).format('MMMM Y');
					} else {
						subtitleText = 'from ' + moment(parseDateTime(startDate)).format('L') + ' ' + moment(parseDateTime(startDate)).format('LTS') + ' to ' + moment(parseDateTime(endDate)).format('L') + ' ' + moment(parseDateTime(endDate)).format('LTS');
					}
				}
				...
				legend.append('rect')
					...
					.attr('height', 8)
					.attr('width', 8)
					.attr('class', 'rect_has_ok');
				
				legend.append('text')
					.attr('x', width + margin.right - 150 + 10)
					.attr('y', paddingTopHeading + 5.0)
					.text('OK')
					.attr('class', 'legend');
				
				legend.append('rect')
					.attr('x', width + margin.right - 150)
					.attr('y', paddingTopHeading + 9)
					.attr('height', 8)
					.attr('width', 8)
					.attr('class', 'rect_has_warning');
				
				legend.append('text')
					.attr('x', width + margin.right - 150 + 10)
					.attr('y', paddingTopHeading + 5.0 + 9 + 0.5)
					.text('WARNING')
					.attr('class', 'legend');
				
				legend.append('rect')
					.attr('x', width + margin.right - 150)
					.attr('y', paddingTopHeading + 18)
					.attr('height', 8)
					.attr('width', 8)
					.attr('class', 'rect_has_fail');
				
				legend.append('text')
					.attr('x', width + margin.right - 150 + 10)
					.attr('y', paddingTopHeading + 5.0 + 18 + 0.5)
					.text('FAIL')
					.attr('class', 'legend');
		});
	}
